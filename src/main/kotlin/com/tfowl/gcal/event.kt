...
class ExtendedPropertiesBuilder(private val map: MutableMap<String, String>) {
    fun prop(pair: Pair<String, String>) = prop(pair.first, pair.second)
...
fun Event.buildSafeExtendedProperties(block: EventExtendedPropertiesBuilder.() -> Unit): Event =
    setExtendedProperties(EventExtendedPropertiesBuilder().also(block).build())

val Event.safeExtendedProperties: Event.ExtendedProperties get() = extendedProperties.unchunk()
